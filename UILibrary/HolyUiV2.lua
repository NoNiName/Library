...
function Library:CreateWindow(HubName, GameName, IntroText, IntroIcon, ImprovePerformance, ConfigFolder, Theme)
    ...
    function Tabs:CreateTab(TabName, DefaultVisibility, Icon, RectOffset, RectSize)
        ...
        function Sections:CreateSection(Name)
            ...
            function Elements:CreateMultiDropdown(Name, ListTable, DefaultTable, DebounceAmount, Callback)
                local Name = Name or 'Dropdown'
                local List = ListTable or {}
                local Callback = Callback or function() end
                local DebounceAmount = DebounceAmount or 0.25
                local Default = DefaultTable or nil
                local SelectedItem = 'None'
                local Opened = false
                local Debounce = false
                local DropdownFunctions = {}
                ...
                for _, Item in next, List do
                    ...
                    OptionButton.MouseButton1Click:Connect(function()
                        ...
                        if table.find(Default,Item) then
                            for i,valueremove in pairs(Default) do
                                if valueremove == Item then
                                    table.remove(Default, i)
                                end
                            end

                            local Str = '';
                            for Idx, Value in next, Default do
                                Str = Str .. Value .. ', ';
                            end;
                            Str = Str:sub(1, #Str - 2);

                            DropdownSelectedText.Text = Str or "None"
                        else
                ...
                if Default ~= nil then
                    for i,v in pairs(Default) do
                        local Option = DropList[tostring(v) .. 'OptionButton']
                        Utility:Tween(Option, {BackgroundColor3 = Utility:Lighten(Theme.PrimaryElementColor)}, 0.25)
                    end
